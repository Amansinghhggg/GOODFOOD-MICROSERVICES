...
const Sales = () => {
  ...
  if (loading) {
    return (
      <div className="flex h-40 items-center justify-center rounded-xl border border-brand-border bg-white shadow-premium-sm">
  ...
  return (
    <div className="space-y-6">
      ...
      {topItems.length > 0 && (
        <div className="rounded-xl border border-brand-border bg-white p-6 shadow-premium">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-base font-bold font-serif text-brand-charcoal">Top Selling Items</h3>
          </div>
              ...
              <div key={item.itemId} className="space-y-1.5">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    <span className="font-medium text-brand-muted">{item.quantity} sold</span>
                    <span className="font-bold text-brand-charcoal">₹{item.revenue.toLocaleString("en-IN")}</span>
                  </div>
                </div>
          ...
          <div className="mt-5 flex flex-wrap gap-2 border-t border-brand-border/60 pt-4">
            {topItems.map((item, idx) => {
              const totalRev = topItems.reduce((s, i) => s + i.revenue, 0);
              const pct = ((item.revenue / totalRev) * 100).toFixed(1);
              return (
                <div key={item.itemId} className="flex items-center gap-1.5 rounded-lg bg-brand-cream/60 border border-brand-border/30 px-3 py-1.5">
                  <span
                    className={`h-2.5 w-2.5 rounded-full bg-gradient-to-br ${RANK_COLORS[idx] ?? "from-brand-primary to-brand-primary-hover"}`}
                  />
                  <span className="text-xs text-brand-muted font-medium">{item.name}</span>
                  <span className="text-xs font-bold text-brand-charcoal">{pct}%</span>
                </div>
              );
            })}
          </div>
      ...
      {/* Empty state */}
      {topItems.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-xl border border-brand-border bg-white shadow-premium py-12">
          <IndianRupee size={32} className="mb-3 text-brand-border" />
          <p className="text-sm font-semibold text-brand-muted">No sales data available yet</p>
        </div>
      )}
    </div>
  );
