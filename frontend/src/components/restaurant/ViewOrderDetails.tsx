      ...
      <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="text-sm font-medium uppercase tracking-wide text-slate-500">Order details</p>
              <h2 className="mt-1 text-2xl font-semibold text-slate-900">{order?.restaurantName ?? "Order Details"}</h2>
              <p className="mt-1 text-sm text-slate-500">Order ID: {orderId}</p>
            </div>
            {order && (
              <span className={`inline-flex w-fit rounded-full px-3 py-1 text-sm font-medium ${statusTone[order.status] ?? "bg-slate-100 text-slate-700"}`}>
          ...
          {order ? (
            <div className="mt-5 space-y-6">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <InfoCard label="Total amount" value={formatMoney(order.totalAmount)} highlight />
                <InfoCard label="Payment status" value={order.paymentStatus} />
                <InfoCard label="Payment method" value={order.paymentMethod} />
                <InfoCard label="Placed at" value={new Date(order.createdAt).toLocaleString()} />
              </div>
              ...
              <div className="rounded-xl border border-slate-200 p-4 sm:p-5">
                ...
                <div className="mt-4 space-y-3">
                  {(order.items ?? []).map((item: any, index: number) => (
                    <div key={index} className="flex items-center justify-between gap-4 rounded-lg bg-slate-50 px-4 py-3">
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                <div className="rounded-xl border border-slate-200 p-4 sm:p-5">
                  ...
                  <div className="mt-4 space-y-3 text-sm">
                    ...
                    <Row label="Platform fee" value={formatMoney(order.platformFee)} />
                    <Row label="Rider amount" value={formatMoney(order.riderAmount)} />
                    <div className="border-t border-slate-200 pt-3">
                      <Row label="Total amount" value={formatMoney(order.totalAmount)} strong />
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-slate-200 p-4 sm:p-5">
                  ...
                  <div className="mt-4 space-y-3 text-sm text-slate-700">
                    <p><span className="font-medium text-slate-900">Address:</span> {order.deliveryAddress.formattedAddress}</p>
                    <p><span className="font-medium text-slate-900">Mobile:</span> {formatCount(order.deliveryAddress.mobile)}</p>
                    <p><span className="font-medium text-slate-900">Distance:</span> {formatCount(order.distance).toFixed(1)} km</p>
                  </div>
                </div>
              </div>
            </div>
