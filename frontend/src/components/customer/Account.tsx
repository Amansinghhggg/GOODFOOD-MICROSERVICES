  ...
  if (!user) {
    return (
      <div className="mx-auto flex min-h-[70vh] max-w-4xl items-center justify-center px-4 py-10">
        <div className="rounded-3xl border border-orange-100 bg-[#2C2923] px-10 py-12 text-center shadow-xl shadow-rose-100/50">
    ...
    <div className="min-h-screen bg-[#22201B] px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div className="relative overflow-hidden rounded-3xl shadow-premium-lg">
          ...
          <div className="absolute -right-16 -top-16 h-64 w-64 rounded-full bg-[#2C2923]/5 blur-3xl" />
          ...
          <div className="relative z-10 flex flex-col gap-5 p-7 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              className="inline-flex w-fit items-center gap-2 rounded-xl border border-white/10 bg-[#2C2923]/10 px-5 py-2.5 text-xs font-bold text-white backdrop-blur-sm transition hover:bg-[#2C2923]/20"
            >
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_1.4fr]">
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-3xl bg-[#2C2923] p-5 shadow-premium border border-[#3A352F]/60">
              <h2 className="mb-4 font-serif text-xs font-bold uppercase tracking-widest text-[#A39B8F]">Profile</h2>
              <div className="space-y-3">
                {[
                  ...
                  <div key={label} className="flex items-center gap-3 rounded-2xl bg-[#22201B] border border-[#3A352F]/40 px-4 py-3">
                    <span className="text-brand-primary">{icon}</span>
                    <div>
                      <p className="text-[9px] font-bold uppercase tracking-widest text-[#A39B8F]">{label}</p>
                      <p className="text-xs font-semibold text-[#EFEBE3] break-all">{value}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="relative overflow-hidden rounded-3xl bg-brand-charcoal p-5 text-white shadow-premium">
              <div className="absolute -right-6 -top-6 h-20 w-20 rounded-full bg-[#2C2923]/5" />
            ...
            <div className="rounded-3xl bg-[#2C2923] p-5 shadow-premium border border-[#3A352F]/60">
              <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-serif text-sm font-bold text-[#EFEBE3]">Your Orders</h2>
                </div>
                {orders.length > 0 && (
              ...
              <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
                {orders.length === 0 ? (
                  <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-[#3A352F]/60 py-8 text-center bg-[#22201B]/35">
                    <Package size={28} className="mb-2 text-[#A39B8F]/40" />
                    <p className="text-sm font-bold text-[#EFEBE3]">No orders yet</p>
                    <p className="text-xs text-[#A39B8F]">Your order history will appear here</p>
                  </div>
                ) : (
                  orders.map((order) => (
                    <Link
                      to={`/order/${order._id}`}
                      key={order._id}
                      className="group flex items-center justify-between gap-3 rounded-2xl border border-[#3A352F]/60 bg-[#22201B]/30 px-4 py-3 transition hover:border-brand-primary/20 hover:bg-brand-primary/5"
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="font-serif text-sm font-bold text-[#EFEBE3] truncate">{order.restaurantName}</p>
                          <span className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider ${STATUS_COLORS[order.status] ?? "bg-[#22201B]-dark text-[#A39B8F]"}`}>
                            {order.status.replace(/_/g, " ")}
                          </span>
                        </div>
                        <p className="mt-0.5 truncate text-xs text-[#A39B8F]">
                          {order.items.slice(0, 2).map((i: any) => i.name).join(", ")}
                          {order.items.length > 2 && ` +${order.items.length - 2} more`}
                        </p>
                        <div className="mt-1 flex items-center gap-1.5 text-[10px] text-[#A39B8F]/70">
                          <Clock size={10} />
                          {new Date(order.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}
                        </div>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-sm font-black text-[#EFEBE3]">₹{order.totalAmount}</p>
                        <ChevronRight size={14} className="ml-auto mt-1 text-[#A39B8F] transition group-hover:text-brand-primary" />
                      </div>
                    </Link>
                  ))
                )}
            ...
            <div className="rounded-3xl bg-[#2C2923] p-5 shadow-premium border border-[#3A352F]/60">
              <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-serif text-sm font-bold text-[#EFEBE3]">Addresses</h2>
                </div>
                <Link
                  to="/AddAddress"
              ...
              {address.length === 0 ? (
                <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-[#3A352F]/60 py-8 text-center bg-[#22201B]/35">
                  ...
                  <p className="text-sm font-bold text-[#EFEBE3]">No saved addresses</p>
                </div>
              ) : (
                <div className="max-h-52 space-y-2 overflow-y-auto pr-1">
                  {address.map((addr) => (
                    <div key={addr._id} className="flex items-start gap-3 rounded-2xl border border-[#3A352F]/40 bg-[#22201B]/30 px-4 py-3">
