...
const SelectRoles = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f7f7f7] px-4 py-12 text-[#111111] sm:px-6 lg:px-8 relative">
      {/* Logout button at top right */}
      <div className="absolute top-6 right-6 sm:top-8 sm:right-8">
        <button
          onClick={handleLogout}
          className="inline-flex items-center gap-2 rounded-full border border-[#e8e8e8] bg-white px-4 py-2 text-xs font-bold text-[#555555] shadow-sm transition hover:border-[#ff385c]/40 hover:bg-[#ff385c]/5 hover:text-[#ff385c]"
        >
          <LogOut size={14} />
          <span>Logout</span>
        </button>
      ...
      <div className="relative mx-auto max-w-5xl mt-8">
        ...
        <div className="mb-14 text-center">
          <div className="mb-4 inline-flex items-center rounded-full border border-[#e8e8e8] bg-white px-4 py-2 text-xs font-semibold text-[#555555] shadow-sm">
            🍔 Fast • Fresh • Delivered
          </div>
          <h1 className="font-serif text-5xl font-black tracking-tight sm:text-6xl text-[#111111]">
          ...
          <div className="mt-10">
            <h2 className="font-serif text-xl font-bold text-[#111111]">Choose your role</h2>
            <p className="mt-1 text-xs text-[#999999]">How would you like to use GOODFOOD?</p>
          </div>
        </div>
                ...
                <button
                  ...
                  className={`relative cursor-pointer rounded-2xl border p-6 text-left transition-all duration-300 hover:-translate-y-1 ${
                    isSelected
                      ...
                      : "border-[#e8e8e8] bg-white hover:border-[#ff385c]/30 hover:shadow-sm"
                  }`}
                >
                  {isSelected && (
                  ...
                  <h3 className={`font-serif text-lg font-bold transition-colors ${isSelected ? "text-[#ff385c]" : "text-[#111111]"}`}>
                    {item.title}
                  </h3>
                  <p className="mt-2 text-xs leading-relaxed text-[#555555] font-medium">{item.description}</p>
                </button>
              );
            })}
          </div>
